---
title: "STAT 420, Summer 2018 - Final Project Proposal"
date: '15-Jul-2018'
output:
  html_document: 
    toc: yes
...
This study will be conducted by the following individuals:
 ...
 - Raymond Ordona (NetID: **xxxx**)
 - Apoorva H Srinivasa (NetID: **xxxx**)
...
For this project we will be utilizing a dataset that contains information on approximately 75,000 different recipies of craft beer along with the readings of various sensors captured during the brewing process. The raw data was sourced from [Brewer's Friend Beer Recipes](https://www.kaggle.com/jtrofe/beer-recipes) available under Kaggle platform.

The dataset has 23 attributes. The response variable - `ABV` is a *real* variable. Below are some of the key attributes (predictors) that we would be using in our study,
...
| `BoilTime`    | Real        | Scale to determine the concentration of dissolved solids in wort |
| `BoilGravity` | Real        | Color using Reference Method - light to dark ex. 40 = black |
...
There are many brew enthusiast who like to brew beer in home. To make a good beer or to improve a receipe over several tries it is essential to capture several readings during and after the brewing process which requires one to buy costly sensors or measuring gadgets. We wanted to analyze and see if we can make predictions of one of the parameters namely - ABV - from other readings without having to buy any measuing gadgets. 

---

## Data Load - Preliminary Check

Below `R` block is an evidence that data can be loaded into `R`. Also printed are the first few values of the response variable **ABV**.
