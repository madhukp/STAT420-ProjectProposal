...
The raw dataset has a total of 23 attributes. The response variable - `ABV` is a *real* variable. Below are all the other attributes (predictors) and their definitions. Attributes that are of no importance for statistical study and which will not be used are also indicated in this table. 

[Have copied from proposal, will add additional column and rows later]
...
Below are the steps performed as part of data preparation/pre-processing,

- Firstly, read raw dataset.
- Removed unwanted columns.
- Removed rows with nulls/`NaN`/`NA`/`N/A` (or any other invalid records).
- (anything else?)
- (anything else?)
**[Lets not split the data here itself, we'll save it for Methodologies and Metrics section.]**

Below is the `R` code for the pseudo code described above,
...
*[For notes only, will be removed: Statistical exploration - Check domain and range of each attribute, max, min, mean, standard deviation and see if anything clicks here]*
...
*[Notes: Visual exploration: draw histograms, boxplots, pairs and comment on data distribution]*
...
```{r}
pairs(train_set[, c("ABV", "FG")], col = "#004e66", pch = 20)
```
...
*[Notes: List out all the methods that will be used, list out math equations if needed. Define and list out all metrics that will be used to evaluate the models - R-squared, RMSE, etc. what all significance levels will be used etc.]*
...
residual_fitted_plot = function(model, data,  outlier, threshold = 0) {
  ...
  plot = ggplot(model) +
          geom_point(aes(x = .fitted, y = .resid), colour = "#004e66") +
  ...
  if (outlier == TRUE ) {
    ...
    outlier_plot = data.frame(fitted = fitted(model)[outlier_index], 
                              resid = resid(model)[outlier_index], 
                              outliers = outlier_index)
    
    plot = plot + geom_point(data = outlier_plot, aes(x = fitted, y = resid, colour = "red")) + 
      scale_color_discrete(name = "", labels = "Outliers") +
      geom_text(data = outlier_plot, 
                aes(x = fitted, y = resid, label = outliers), 
                check_overlap = TRUE, 
                hjust = 0, 
                nudge_x = 0.5)
...
```{r}
df = data.frame(x = predicted_abv, y = actual_abv)
ggplot(df, aes(x = x, y = y)) +
  geom_point(colour = "#004e66") +
...
qqnorm(resid(model3), col = "#004e66", pch = 20)
